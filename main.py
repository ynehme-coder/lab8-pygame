...
def create_random_square() -> Square:
    ...
    Speed is mapped linearly from size using the current constants/formula.
    """
    # Updated docstring so comments match the implemented formula.
...
def get_square_center(square: Square) -> tuple[float, float]:
    """Return the (x, y) center of a square."""
    # Centralizing repeated coordinate math follows DRY and improves readability.
    return square.x + square.size / 2, square.y + square.size / 2


def clamp_velocity(square: Square) -> None:
    """Limit velocity magnitude to square.max_speed."""
    # Shared helper keeps speed-clamp behavior in one place (single responsibility).
...
def bounce_on_walls(square: Square) -> None:
    """Clamp square to screen bounds and bounce on collision with edges."""
    if square.x <= 0:
        square.x = 0
        square.vx *= -1
...
def apply_flee_behavior(square: Square, all_squares: list[Square]) -> None:
    ...
    for predator in all_squares:
        # Expanded variable names make intent easier to read for beginners.
        if predator == square or predator.size <= square.size:
            continue
        else:
            predator_center_x, predator_center_y = get_square_center(predator)

            dx = square_center_x - predator_center_x
            dy = square_center_y - predator_center_y
            distance = sqrt(dx**2 + dy**2)

            if distance <= flee_radius and distance != 0:
                # Stronger when closer
                weight = (flee_radius - distance) / flee_radius

                # Normalize direction
                push_x += (dx / distance) * weight
                push_y += (dy / distance) * weight
...
def apply_chase_behavior(square: Square, all_squares: list[Square]) -> None:
    ...
    center_x, center_y = get_square_center(square)

    for candidate in all_squares:
        if candidate == square or candidate.size >= square.size:
            continue

        candidate_x, candidate_y = get_square_center(candidate)

        dx = candidate_x - center_x
        dy = candidate_y - center_y
        distance = sqrt(dx**2 + dy**2)

        if distance < min_distance and distance < chase_radius:
            min_distance = distance
            target = candidate

    if target:
        target_x, target_y = get_square_center(target)
        ...
        if distance != 0:
            square.vx += (dx / distance) * chase_strength
            square.vy += (dy / distance) * chase_strength
...
def handle_input(
    ...
    if event.type == pygame.KEYDOWN:
        ...
        # Re-enabled to keep keyboard behavior consistent with UI/README hints.
        elif event.key == pygame.K_UP:
            target_fps = min(MAX_FPS, target_fps + 10)
        elif event.key == pygame.K_DOWN:
            target_fps = max(MIN_FPS, target_fps - 10)
