...
class Boid:
    ...
    def _random_steer(self, spread: float = 0.2) -> None:
        ...
        angle_change = random.uniform(-spread, spread)
    ...
    def _alignment(self, boids: List['Boid']) -> pygame.Vector2:
        ...
        if count > 0:
            steer /= count
            current_velocity = pygame.Vector2(self.vx, self.vy)
    ...
    def update(self, boids: List['Boid'], dt: int) -> None:
        ...
        if config.ALIGNEMENT_ON:
            ...
            self.vx += alignment_force.x * config.ALIGNEMENT_STEER_STRENGTH * dt_seconds
            self.vy += alignment_force.y * config.ALIGNEMENT_STEER_STRENGTH * dt_seconds
